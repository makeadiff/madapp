...
class Classes extends Controller {
    ...
    /*
     * Function Name : index()
     * Wroking :Shows all the classes the current user is resposible for.
     * @author:
     * @param :[$user_id]
     * @return: type: []
     */
    ...
    /*
     * Function Name : batch_view()
     * Wroking :This is the batch head view. Sees the entire options of a batch at once.
     * @author:
     * @param :[$batch_id,$from_date,$to_date]
     * @return: type: []
     */
    ...
    function batch_view($batch_id=0, $from_date='', $to_date='') {
        ...
        if (!$batch_id)
            $batch_id = $this->user_model->get_users_batch($this->user_details->id);
        if (!$batch_id) {
        ...
        foreach ($data as $row) {
            ...
            foreach ($attendence as $id => $status)
                if ($status == 1)
                    $present_count++;
        ...
        $this->load->view('classes/batch_view', array('classes' => $classes, 'center_name' => $center_name, 'batch_id' => $batch_id, 'batch_name' => $batch->name, 'from_date' => $from_date, 'to_date' => $to_date,
            'all_lessons' => $all_lessons, 'all_user_names' => $all_user_names));
        //$this->load->view('layout/footer');
    }

    /*
     * Function Name : batch_view_save()
     * Wroking :This is the batch head to save all batch activities.
     * @author:Rabeesh
     * @param :[]
     * @return: type: []
     */
     ...
     * Wroking :This function used for showing the marking attendance view .
     * @author:
     * @param :[$class_id]
     * @return: type: []
     */
    ...
    /*
     * Function Name : mark_attendence()
     * Wroking :This function used for saving the marked attendance.
     * @author:
     * @param :[]
     * @return: type: []
     */
     ...
     * Wroking :Cancel a class - the id must be given as the argument. If the batch_id argument is provided, goes batch to that batch's view.
     * @author:
     * @param :[$class_id,$batch_id,$date]
     * @return: type: []
     */
     ...
     * Wroking :Un-Cancel a class - If the user cancels a class by accident, do this.The id must be given as the argument.
     * @author:
     * @param :[$class_id,$batch_id,$date]
     * @return: type: []
     */
    ...
    /*
     * Function Name : madsheet()
     * Wroking :Creating the MADSheet in User mode.
     * @author:
     * @param :[]
     * @return: type: []
     */
            ...
            foreach ($batches as $batch_id => $batch_name) {
                ...
                foreach ($all_levels[$center->id] as $level) {
                    ...
                    foreach ($all_classes as $class) {
                        if (isset($teachers_info[$class->user_id]))
                            continue;
                    ...
                    foreach ($teachers_info as $teacher_id => $teacher_name) {
                        foreach ($all_classes as $class) {
                            ...
                            if (!in_array($date, $days_with_classes)) {
                                ...
                                if ($month <= 3)
                                    $month = $month + 12; // So that january comes after december.
    ...
    /*
     * Function Name : madsheet_class_mode()
     * Wroking :MADSheet in Class Mode.
     * @author:
     * @param :[]
     * @return: type: []
     */
    ...
    /*
     * Function Name : edit_class()
     * Wroking :First see if the user takes this class. Then he has premission to edit it.
     * @author:
     * @param :[$class_id,$type]
     * @return: type: []
     */
    ...
    function edit_class($class_id, $type=false) {
        ...
        $this->load->view('classes/form', array('class_details' => $class_details, 'teachers' => $teachers, 'substitutes' => $substitutes,
            'statuses' => $statuses, 'message' => $this->message,
            'all_lessons' => $all_lessons, 'from' => $type));
    }

    /*
     * Function Name : edit_class_save()
     * Wroking :Saving the editted classes.
     * @author:
     * @param :[]
     * @return: type: []
     */
    ...
    function edit_class_save() {
        ...
        for ($i = 0; $i < $teacher_count; $i++) {
            ...
            if ($teacher_ids[$i] == $substitute_ids[$i])
                $substitute_ids[$i] = 0; // To make sure a person can't substitute for himself.
     ...
     * Wroking :This function is used for Confirm the class
     * @author:
     * @param :[$class_id]
     * @return: type: []
     */
    ...
    /*
     * Function Name : add_manually()
     * Wroking :Showing the add window for adding classes manualy
     * @author:
     * @param :[$batch_id,$center_id]
     * @return: type: []
     */
    ...
    /*
     * Function Name : add_manually_save()
     * Wroking :Saving the classes manualy into database
     * @author:
     * @param :[]
     * @return: type: []
     */
    ...
    /*
     * Function Name : other_city_teachers()
     * Wroking :This function used for getting other city teachers
     * @author:
     * @param :[$flag]
     * @return: type: []
     */
    ...
    /*
     * Function Name : city_teachers()
     * Wroking :Getting city teachers
     * @author:
     * @param :[$city_id,$flag]
     * @return: type: []
     */
    ...
    /*
     * Function Name : update_city_teachers()
     * Wroking :This function used for updating city teachers details.
     * @author:
     * @param :[$user_id,$flag]
     * @return: type: []
     */
     ...
     * Wroking :This function used for deleting classes.
     * @author:
     * @param :[$class_id]
     * @return: type: []
     */
