...
class Debug extends Controller {
	...
	public function update_class_date() {
		$from_date = '2018-10-01';
		$to_date = '2018-10-18';

		$classes = $this->batch_model->db->query("SELECT * FROM Class WHERE DATE(class_on) BETWEEN '$from_date' AND '$to_date'")->result();
		print "Classes : " . count($classes) . "<br />";
		...
		foreach($classes as $cls) {
			if(!isset($batches[$cls->batch_id])) {
				$batches[$cls->batch_id] = $this->batch_model->get_batch($cls->batch_id);
			}

			$day = $batches[$cls->batch_id]->day;

			$class_day = date('w', strtotime($cls->class_on));
			$count['total']++;

			if($day != $class_day) {
				$count['different']++;

				$difference = $day - $class_day;
				$new_date = date_add(date_create($cls->class_on), date_interval_create_from_date_string("- $difference days"));
				$new_date_text = $new_date->format("Y-m-d H:i:s");

				$existing = $this->batch_model->db->query("SELECT id FROM Class WHERE class_on='$new_date_text' AND batch_id={$cls->batch_id} AND level_id={$cls->level_id}")->row();

				if(!$existing) {
					$this->batch_model->db->query("UPDATE Class SET class_on='$new_date_text' WHERE id={$cls->id}");
					$count['updated']++;

					echo $cls->id . ") " . $cls->class_on . ' => ' . $new_date_text . '<br />';
				} else {
					$difference = $class_day - $day;
					$new_date = date_add(date_create($cls->class_on), date_interval_create_from_date_string("- $difference days"));
					$new_date_text = $new_date->format("Y-m-d H:i:s");
					...
					if(!$existing) {
						...
						print "Deleting {$cls->id}<br />";
						$this->batch_model->db->query("DELETE FROM Class WHERE id={$cls->id}");
						$count['deleted']++;
					}
				}
			}
	...
	public function bulk_delete_class() {
		...
		$classes = $this->batch_model->db->query("SELECT id FROM Class WHERE DATE(class_on) BETWEEN '$from_date' AND '$to_date'")->result();
		foreach ($classes as $cls) {
			print "Deleted {$cls->id}<br />";
			$this->batch_model->db->query("DELETE FROM Class WHERE id={$cls->id}");
			$this->batch_model->db->query("DELETE FROM UserClass WHERE class_id={$cls->id}");
			$this->batch_model->db->query("DELETE FROM StudentClass WHERE class_id={$cls->id}");
		}
