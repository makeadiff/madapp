...
class imageResize 
{
    ...
    public static function Resize($imagename,$width,$height)
	{
		...
		{
			$src_image = imagecreatefromjpeg($imagename);
		}
		...
		{
			$src_image = imagecreatefromgif($imagename);
		}
		elseif(preg_match("/bmp|BMP/", end($extension)))
		{
			$src_image = imagecreatefrombmp($imagename);
		}
		...
		$dest_image = imagecreatetruecolor($thumb_w, $thumb_h);
		imagecopyresized($dest_image, $src_image, 0, 0, 0, 0, $thumb_w, $thumb_h, $old_x, $old_y);
...
*  $watermarklogo this is for watermark logo , $watermarkimage this is for source image to be watermarked
*  caution:source file will be replaced(if u dont need to replace sourc file then edit this linne imagejpeg($image,$watermarkimage);  )
*/	
	
	function create_watermark($watermarklogo,$watermarkimage) {
	
	$watermark = imagecreatefromgif($watermarklogo);  
	$watermark_width = imagesx($watermark);  
	$watermark_height = imagesy($watermark);  
	$image = imagecreatetruecolor($watermark_width, $watermark_height);  
	$image = imagecreatefromjpeg($watermarkimage);  
	...
	imagejpeg($image,$watermarkimage);  
	imagedestroy($image);  
