...
class Users_model extends Model {
	private $credit_for_substituting = false;
	private $credit_lost_for_getting_substitute = false;
	private $credit_lost_for_missing_class = false;
	private $credit_lost_for_missing_zero_hour = false;
	private $credit_max_credit_threshold = false;
  ...
  function get_sofs_in_city($city_id=false){
    ...
    function cache_credit_settings($vertical_prefix) {
		...
		$this->credit_lost_for_getting_substitute = floatval($this->ci->settings_model->get_setting_value($vertical_prefix . 'credit_lost_for_getting_substitute'));
		$this->credit_lost_for_missing_class = floatval($this->ci->settings_model->get_setting_value($vertical_prefix . 'credit_lost_for_missing_class'));
    ...
    function recalculate_user_credit($user_id, $update_if_wrong=false, $debug=false) {
		...
		$credit_lost_for_missing_zero_hour = $this->credit_lost_for_missing_zero_hour;
		$credit_max_credit_threshold = $this->credit_max_credit_threshold;
		$credit = $this->credit;
	...
	function get_class_connections($user_id) {
		$mentor_at = $this->db->query("SELECT DISTINCT B.id AS batch_id, B.day, B.class_time, C.class_on, Ctr.id AS center_id, Ctr.name AS center_name, 
				B.project_id, P.name AS project
			FROM Batch B
		...
		$teacher_at = $this->db->query("SELECT B.id AS batch_id, UB.level_id, CONCAT(L.grade, ' ', L.name) AS level, B.day, B.class_time, C.id AS class_id, C.class_on,
												Ctr.id AS center_id, Ctr.name AS center_name, B.project_id, P.name AS project
			FROM UserBatch UB
			...
			WHERE B.status='1' AND UB.user_id='$user_id' AND UC.user_id=$user_id AND B.year='{$this->year}' AND L.year={$this->year} AND C.class_on < NOW()
				AND C.class_on=(SELECT class_on FROM Class WHERE batch_id=B.id AND level_id=L.id AND class_on < NOW() AND status!='cancelled'
									ORDER BY class_on
