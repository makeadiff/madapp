...
<div style="float:left; margin-top:20px;">
    <form id="formEditor" class="mainForm clear" action="<?php echo site_url('placement/update_calendenar_event_name/' . $root_id) ?>" method="post" onsubmit="return validate();" style="width:355px;">
        <input type="hidden" name="started_date" id="started_date" value="<?php echo $started_date; ?>">
...
if($row['id']) { ?>
    ...
    <input type="checkbox" id="attendence_<?php echo $row['id'] ?>" name="attendance[]"  value="<?php echo $row['id'] ?>"  <?php foreach($student->result_array() as $student_array){ if($student_array['student_id'] == $row['id']){?>checked="checked"<?php }}?>/>
    </li>
    <?php } } ?>
...
?>
            ...
            <ul>
                <li><input id="btnSubmit"  class="button green" type="submit" value="Submit"  />
                    <a href="<?= site_url('placement/popuplistevents/' . $started_date) ?>" class="cancel-button">Cancel</a>
                </li>
            </ul>
        </fieldset>
    </form>		
